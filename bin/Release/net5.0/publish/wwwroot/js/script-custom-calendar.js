        ...
        if (calendarEl != null) {
            calendar = new FullCalendar.Calendar(calendarEl, {
                ...
                events: function (fetchInfo, successCallback, failureCallback) {
                    $.ajax({
                        url: routeURL + '/api/Appointment/GetCalendarData?therapistId=' + $("#therapistId").val(),
                        type: 'GET',
                        dataType: 'JSON',
                            ...
                            if (response.status === 1) {
                                $.each(response.dataenum, function (i, data) {
                                    events.push({
                                        ...
                                        backgroundColor: data.isTherapistApproved ? "#28a745" : "#dc3545",
                                        borderColor: "#162466",
                                        textColor: "white",
                                        id: data.id
                                    });
...
function onShowModal(obj, isEventDetail) {
    if (isEventDetail != null) {
        ...
        $("#therapistId").val(obj.therapistId);
        $("#patientId").val(obj.patientId);
        $("#id").val(obj.id);
        $("#lblPatientName").html(obj.patientName);
        $("#lblTherapistName").html(obj.therapistName);
        if (obj.isTherapistApproved) {
